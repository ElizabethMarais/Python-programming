...
print " "
print "PRINT rows in Invoices table (first 6)."
print "Acc no Invoice no   Project No.   Project Name       Client                    Date Inv.    Amount"
sqlstr = 'SELECT * FROM EngProjects LIMIT 6'   # order by year ASC'
for row in cur.execute(sqlstr):
     print row[0], " ", row[1], "  ", row[2], "  ",  row[3], "   ", row[4], "  ", row[5], "  ",  row[6]

print " "
print "PRINT rows in Clients table (first 6)."
print "Client Name   Address                       VAT_No    Contact person    Email address" 
sqlstr = 'SELECT * FROM EngClients order by name LIMIT 6'
for row in cur.execute(sqlstr):
     print row[0], " ", row[1], "  ", row[2], "  ",  row[3], "   ", row[4]

print " "
#this method uses items from the list and combine it with SQL
print "PRINT information from Invoice table, added the Client VAT no. & email address, looked up from Clients table. "
print "Invoice no   ProjectNo.  Project Name       Client                    Date Inv.    Amount    VAT no.   EmailAddress"
...
print "PRINT information from Invoice table, added the Client VAT no. & email address, looked up from Clients table using COMBINED SQL STATEMENTS. "
print "Invoice no.   Amount  ProjectNo. Client                 VAT no.   EmailAddress"
sqlstr = "SELECT EngProjects.Invoice_no, EngProjects.Amount,  EngProjects.Proj_no, EngProjects.Client, EngClients.VAT_no,  EngClients.Email_address FROM EngProjects JOIN EngClients ON EngProjects.Client = EngClients.name"
for row in cur.execute(sqlstr):
    amount = "R" + str(row[1])
    print row[0], "  ", amount, "  ", row[2], "  ",  row[3], "   ", row[4],"   ", row[5]

#EngProjects (Acc_no TEXT, Invoice_no TEXT, Proj_no TEXT, Proj_name TEXT, Client TEXT, Date_Inv TEXT, Amount INTEGER)''')

#EngClients (name TEXT, address TEXT, VAT_No INTEGER, Contact_person TEXT, Email_address TEXT)''')
